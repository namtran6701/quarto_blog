...
fig.add_trace(go.Scatter(x=msft.index, y=msft['Adj Close'], mode='lines', name='MSFT'), row=1, col=1)

# Add bar plot for volume (the second row)
fig.add_trace(go.Bar(x=msft.index, y=msft['Volume'], name='Volume', marker_color='red'), row=2, col=1)

# Set layout
fig.update_layout(height=600, width=800, title_text="MSFT Adjusted Price and Volume", template = 'plotly_white', showlegend= True)
...
fig.add_trace(go.Scatter(x = nvda.index, y = nvda['Adj Close'], mode = 'lines', name = 'NVDA'), row = 2, col = 1)

# add the line plot for GOOG 

fig.add_trace(go.Scatter(x = goog.index, y = goog['Adj Close'], mode = 'lines', name = 'GOOG'), row = 3, col = 1)

# set the layout

fig.update_layout(height = 800, width = 1000, title_text = "MSFT, NVDA, and GOOG Adjusted Price Since The Introduction of ChatGPT", template = 'plotly_white', showlegend = True)
